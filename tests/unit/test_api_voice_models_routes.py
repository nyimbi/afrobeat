...
def _make_voice_model(
	...
	vm.created_at = datetime(2025, 1, 1, tzinfo=timezone.utc)
	vm.deleted_at = None
	return vm
...
def test_list_voice_models_returns_presets_and_custom():
...
def test_list_voice_models_no_custom_returns_only_presets():
	client, mock_db, _ = _build_client()
...
def test_get_voice_model_status_owned_returns_200():
	client, mock_db, _ = _build_client()
...
def test_get_voice_model_status_not_found_returns_404():
	client, mock_db, _ = _build_client()
...
def test_get_voice_model_status_other_user_custom_returns_403():
	client, mock_db, _ = _build_client()
...
def test_get_voice_model_status_preset_accessible_by_any_user():
	client, mock_db, _ = _build_client()
...
def test_delete_voice_model_owned_returns_200():
	client, mock_db, _ = _build_client()
	vm = _make_voice_model()
	vm.delete = AsyncMock()
...
def test_delete_voice_model_not_found_returns_404():
	client, mock_db, _ = _build_client()
...
def test_delete_voice_model_preset_returns_403():
	client, mock_db, _ = _build_client()
...
def test_delete_voice_model_not_owned_returns_403():
	client, mock_db, _ = _build_client()
	vm = _make_voice_model(user_id="other-user-999", is_preset=False)
...
def test_upload_voice_sample_unsupported_type_returns_422():
	from gbedu_api.main import app
...
def test_upload_voice_sample_free_tier_returns_403():
	from gbedu_api.main import app
...
def test_upload_voice_sample_pro_tier_accepted():
	from gbedu_api.main import app
	from gbedu_api.deps import get_db, get_current_active_user, get_storage, require_tier
	...
	with patch("gbedu_api.routers.voice_models.VoiceModel", return_value=mock_vm), \
